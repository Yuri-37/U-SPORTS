...
import { getMatchReviewData, resolveParticipantDisplayName, actionTypeToStatKeys } from '../services/matchReviewData'
...
async function aggregateOfficialSeasonStatsForMatch(matchId: string, winnerId: string): Promise<string | null> {
...
router.post('/:matchId/action', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  ...
  try {
    ...
    if (body.participantId !== match.participant_a_id && body.participantId !== match.participant_b_id) {
    ...
    const opponentId = body.participantId === match.participant_a_id
      ? match.participant_b_id
      : match.participant_a_id
    const scoringParticipantId = effect.target === 'own' ? body.participantId : opponentId

    // Log the action
    const { data: insertedRows, error: insertError } = await supabase.from('scoring_actions').insert({
      match_id: matchId,
    ...
    if (effect.scores && scoringParticipantId) {
      ...
      if (insertedId) {
        const { data: afterScores } = await supabase
          .from('match_scores')
          .select('participant_id, q1, q2, q3, q4, ot, set1, set2, set3, set4, set5, game1, game2, game3, game4, game5')
          .eq('match_id', matchId)
        if (afterScores) {
          await supabase.from('scoring_actions').update({ score_after: afterScores }).eq('id', insertedId)
        }
      }
    }
...
router.patch('/:matchId/period', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  ...
  try {
    ...
    if (await respondIfSportForbidden(req, res, sport)) return
    if (period > maxPeriodFor(sport)) {
      return res.status(400).json({ error: `${sport} has at most ${maxPeriodFor(sport)} periods` })
    }
    ...
    res.status(400).json({ error: err instanceof Error ? err.message : 'Could not change period' })
  }
})
...
// was left running.
router.patch('/:matchId/clock', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  const matchId = req.params.matchId as string
  const schema = z.object({
...
router.patch('/:matchId/first-server', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  const matchId = req.params.matchId as string
  ...
  try {
    ...
    res.status(400).json({ error: err instanceof Error ? err.message : 'Could not set first server' })
  }
})

// Validate (close) a period. Freezes its scores and blocks further writes to it.
router.post('/:matchId/period/:period/validate', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  const matchId = req.params.matchId as string
  ...
  try {
    ...
    if (!field) return res.status(400).json({ error: `Period ${period} is not valid for ${sport}` })
    ...
    if (lockErr) {
      if (lockErr.code === '23505') return res.status(409).json({ error: `Period ${period} is already validated` })
    ...
    res.status(400).json({ error: err instanceof Error ? err.message : 'Could not validate period' })
  }
})
...
router.delete('/:matchId/period/:period/validate', requireAuth, requireRole('Admin'), async (req: AuthRequest, res) => {
  const matchId = req.params.matchId as string
  const period = Number(req.params.period)
  const reason = typeof req.body?.reason === 'string' ? req.body.reason.trim() : ''

  if (!Number.isInteger(period)) return res.status(400).json({ error: 'Invalid period' })
  if (reason.length < 3) return res.status(400).json({ error: 'A reason is required to reopen a validated period' })
...
router.post('/:matchId/undo', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  ...
  if (!actions || actions.length === 0) return res.status(404).json({ error: 'No action to undo' })
  ...
  const targetAction = (actions as Record<string, unknown>[]).find((a) => {
    ...
    const scoring = effect.target === 'own'
      ? actingPid
      : (actingPid === match?.participant_a_id ? match?.participant_b_id : match?.participant_a_id)
    return scoring === scoringParticipantId
  })

  if (!targetAction) {
    return res.status(404).json({
      error: scoringParticipantId ? 'No scoreable action to undo for this team' : 'No scoreable action to undo',
    })
  }
  ...
  if (effect.scores && actingParticipantId && match) {
    const opponentId = actingParticipantId === match.participant_a_id
      ? match.participant_b_id
      : match.participant_a_id
  ...
  if (athleteId) {
    await updatePlayerGameStats(matchId, athleteId, sport, String(targetAction.action_type ?? ''), -1)
  }
...
router.post('/:matchId/end', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const seasonId = await aggregateOfficialSeasonStatsForMatch(req.params.matchId as string, winnerId)
...
router.get('/:matchId/state', async (req, res) => {
  ...
  const matchDecision = computeMatchDecision(sport, scoreRowA, scoreRowB, lockedPeriods, eventInfo?.bestOf ?? null, aId, bId)
...
const ALLOWED_ACTIONS: Record<string, readonly string[]> = {
  basketball: [
    ...
    'rebound', 'off_rebound', 'assist', 'steal', 'block', 'turnover', 'foul',
  ],
  volleyball: ['point_1', 'kill', 'ace', 'block', 'dig', 'assist', 'error', 'serve_error', 'reception_error', 'timeout'],
...
async function getLockedPeriods(matchId: string): Promise<number[]> {
  const { data } = await supabase.from('match_period_locks').select('period').eq('match_id', matchId)
...
async function getMatchSport(matchId: string): Promise<string | null> {
  ...
  const { data: e } = await supabase.from('events').select('sport').eq('id', m.event_id).maybeSingle()
  return e?.sport ?? null
}
...
function pointEffect(sport: string, actionType: string): { scores: boolean; target: 'own' | 'opponent'; points: number } {
  if (sport === 'basketball') {
  ...
  if (sport === 'volleyball') {
    if (actionType === 'kill' || actionType === 'ace' || actionType === 'block' || actionType === 'point_1') {
      return { scores: true, target: 'own', points: 1 }
    }
    if (actionType === 'error' || actionType === 'serve_error' || actionType === 'reception_error') {
...
async function recomputePeriodWins(
  ...
  const { data: scoresData } = await supabase
    .from('match_scores')
    .select('participant_id, set1, set2, set3, set4, set5, game1, game2, game3, game4, game5, game6, game7')
    .eq('match_id', matchId)
...
async function getMatchEventInfo(
  ...
  return { sport: e.sport, bestOf: (e as { best_of?: number | null }).best_of ?? null, ttFormat: e.table_tennis_format ?? null }
}
...
function computeMatchDecision(
  ...
  aId: string | null,
  bId: string | null,
): { decided: boolean; winnerParticipantId: string | null; winsA: number; winsB: number; needsOvertime: boolean; reason: string } {
  const empty = { decided: false, winnerParticipantId: null, winsA: 0, winsB: 0, needsOvertime: false, reason: '' }
  ...
  if (sport === 'basketball') {
    ...
    return { decided: false, winnerParticipantId: null, winsA: totalA, winsB: totalB, needsOvertime: true, reason: 'Tied after regulation' }
  }
...
async function deriveServer(
  ...
  aId: string | null,
  bId: string | null,
): Promise<{ servingParticipantId: string | null; source: 'derived' | 'first-server-fallback' | 'unset' }> {
  if (sport !== 'volleyball' && sport !== 'table-tennis') return { servingParticipantId: null, source: 'unset' }
  ...
  const lastScoring = (rows ?? []).find((r) => pointEffect(String(r.sport), String(r.action_type)).scores)
  if (!lastScoring) return { servingParticipantId: periodFirstServer, source: 'first-server-fallback' }
...
async function fouledOutAthletes(matchId: string): Promise<string[]> {
  const { data } = await supabase.from('player_game_stats').select('athlete_id, stats').eq('match_id', matchId)
  return (data ?? [])
    .filter((r) => Number((r.stats as Record<string, number> | null)?.fouls ?? 0) >= GAME_LIMITS.personalFouls)
...
router.get('/finalized-matches', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  const parsed = finalizedMatchesQuerySchema.safeParse(req.query)
  if (!parsed.success) return res.status(400).json({ error: parsed.error.flatten() })
  const { seasonId, sport } = parsed.data

  const { data: events } = await supabase.from('events').select('id, name').eq('season_id', seasonId).eq('sport', sport)
  const eventMap = new Map((events ?? []).map((e) => [e.id, e.name]))
  if (eventMap.size === 0) return res.json({ matches: [] })

  const { data: matches } = await supabase
    .from('matches')
    .select('id, event_id, bracket_id, participant_a_id, participant_b_id, scheduled_at, venue, finalized_at, scores:match_scores(*)')
  ...
  const { data: brackets } = bracketIds.length
    ? await supabase.from('brackets').select('id, round, bracket_type, winner_id').in('id', bracketIds)
    : { data: [] }
...
router.patch('/:matchId/player-stats/:athleteId', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  ...
  try {
    ...
    if (mrow.status !== 'completed') {
      return res.status(400).json({ error: 'Only a completed match can have its stats corrected' })
    }
    ...
    if (evt.season_id) {
      const { data: season } = await supabase.from('seasons').select('status').eq('id', evt.season_id).maybeSingle()
      if (season?.status === 'archived') {
        return res.status(409).json({ error: 'This season is archived and its results can no longer be edited' })
      }
    }

    // The athlete must actually have played in this match.
    const onA = mrow.participant_a_id ? await isAthleteOnParticipant(mrow.participant_a_id, athleteId as string) : false
    const onB = !onA && mrow.participant_b_id ? await isAthleteOnParticipant(mrow.participant_b_id, athleteId as string) : false
    if (!onA && !onB) {
...
router.patch('/:matchId/lineup', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  ...
  const { data: match } = await supabase
    .from('matches')
    .select('status, active_lineup, participant_a_id, participant_b_id, event_id, scoring_locked_by, current_period')
    .eq('id', matchId)
    .single()
