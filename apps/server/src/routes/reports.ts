...
import { summarizeFinalScore, buildPeriodTable, BOX_SCORE_COLUMNS, type Sport } from '../utils/matchScorePresentation'
...
router.get('/analytics/csv', requireAuth, requireRole('Organizer', 'Admin', 'Coach'), async (req: AuthRequest, res) => {
  ...
  if (!seasonId || seasonId === 'current') {
    const { data: active } = await supabase.from('seasons').select('id, name').eq('status', 'active').limit(1).maybeSingle()
    if (!active?.id) return res.status(400).json({ error: 'No active season configured.' })
    seasonId = active.id
    seasonName = active.name ?? seasonName
  } else {
    const { data: exists } = await supabase.from('seasons').select('id, name').eq('id', seasonId).maybeSingle()
  ...
  try {
    if (tab === 'leaderboard') {
      ...
      sendCsv(res, `${filenamePrefix}-leaderboard-${dateStamp}.csv`, headers, rows as Record<string, unknown>[])
      return
    }
    ...
    if (tab === 'teams') {
      ...
      sendCsv(res, `${filenamePrefix}-teams-${dateStamp}.csv`, ['team_name', 'sport', 'wins', 'losses', 'games_decided', 'win_pct'], rows)
      return
    }
    ...
    sendCsv(res, `${filenamePrefix}-event-results-${dateStamp}.csv`, ['event_name', 'champion', 'runner_up'], rowsNamed)
  } catch (e: unknown) {
...
router.get('/analytics/insights-xlsx', requireAuth, requireRole('Organizer', 'Admin', 'Coach'), async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const snapshots = buildSeasonAggregateInsights(sport, (lbRes.data ?? []) as any, (tsRes.data ?? []) as any)
    ...
    const rows = filter === 'all' ? allRows : allRows.filter((r) => INSIGHT_FILTER_GROUPS[filter]?.includes(r.insight_type))
    ...
    const counts = {
      ...
      team: allRows.filter((r) => r.insight_type === 'streak' || r.insight_type === 'first_win').length,
    }
    ...
    summarySheet.addRow([`${seasonName} — ${titleCase(sport)} Insights Summary`]).font = { bold: true, size: 14 }
    summarySheet.addRow([`Generated: ${new Date().toLocaleDateString('en-PH')}`])
    if (filter !== 'all') summarySheet.addRow([`Detail sheet filtered to: ${FILTER_LABELS[filter]}`]).font = { italic: true }
    summarySheet.addRow([])
    ...
    const aiSheetName = filter === 'all' ? 'Automated Insights' : `Automated Insights — ${FILTER_LABELS[filter]}`
    ...
    for (const row of rows) {
      ...
      if (row.insight_type === 'trending_up') excelRow.getCell('insight_type').font = { bold: true, color: { argb: 'FF16A34A' } }
      if (row.insight_type === 'trending_down') excelRow.getCell('insight_type').font = { bold: true, color: { argb: 'FFDC2626' } }
    }
    if (rows.length === 0) {
      aiSheet.addRow({
        insight_text:
...
function drawScoreGrid(
  ...
  const cell = (x: number, w: number, h: number, text: string, bold: boolean, fontSize: number) => {
    ...
    doc.font(bold ? 'Helvetica-Bold' : 'Helvetica').fontSize(fontSize).text(text, x + 3, y + (h - fontSize) / 2, { width: w - 6, align: 'center' })
  }

  cell(startX, labelWidth, headerRowHeight, 'Team', true, 8)
  columnLabels.forEach((label, i) => cell(startX + labelWidth + i * colWidth, colWidth, headerRowHeight, label, true, 8))
  cell(startX + labelWidth + columnLabels.length * colWidth, colWidth, headerRowHeight, finalLabel, true, 8)
  y += headerRowHeight
  ...
  for (const row of rows) {
    ...
    row.values.forEach((v, i) => cell(startX + labelWidth + i * colWidth, colWidth, dataRowHeight, String(v), false, 9))
    cell(startX + labelWidth + row.values.length * colWidth, colWidth, dataRowHeight, String(row.final), true, 9)
    y += dataRowHeight
  }
...
router.get('/match/:matchId/pdf', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  ...
  doc.text(`Date: ${data.match.scheduled_at ? new Date(data.match.scheduled_at).toLocaleString('en-PH') : '—'}`)
  ...
  doc
    .fontSize(12)
    .font('Helvetica-Bold')
    .text(`${data.participantNames.a} ${summarizeFinalScore(sport, scoreA, scoreB)} ${data.participantNames.b}`, { align: 'center' })
  doc.moveDown(0.5)
  ...
  const finalColumnLabel = sport === 'volleyball' ? 'SETS WON' : sport === 'table-tennis' ? 'GAMES WON' : 'FINAL'
  const finalA =
    sport === 'volleyball' ? Number(scoreA?.sets_won ?? 0)
    : sport === 'table-tennis' ? Number(scoreA?.games_won ?? 0)
    : Number(scoreA?.q1 ?? 0) + Number(scoreA?.q2 ?? 0) + Number(scoreA?.q3 ?? 0) + Number(scoreA?.q4 ?? 0) + Number(scoreA?.ot ?? 0) + Number(scoreA?.ot2 ?? 0) + Number(scoreA?.ot3 ?? 0)
  const finalB =
    sport === 'volleyball' ? Number(scoreB?.sets_won ?? 0)
    : sport === 'table-tennis' ? Number(scoreB?.games_won ?? 0)
    : Number(scoreB?.q1 ?? 0) + Number(scoreB?.q2 ?? 0) + Number(scoreB?.q3 ?? 0) + Number(scoreB?.q4 ?? 0) + Number(scoreB?.ot ?? 0) + Number(scoreB?.ot2 ?? 0) + Number(scoreB?.ot3 ?? 0)

  if (periods.length > 0) {
    drawScoreGrid(doc, periods.map((p) => p.label), finalColumnLabel, [
      { label: data.participantNames.a, values: periods.map((p) => p.left), final: finalA, isWinner: winnerId === data.match.participant_a_id },
      { label: data.participantNames.b, values: periods.map((p) => p.right), final: finalB, isWinner: winnerId === data.match.participant_b_id },
    ])
  }
  ...
  doc.lineWidth(1)
  doc.fontSize(9).font('Helvetica-Bold').text('VICTORIOUS TEAM', 50, bannerY + 8, { width: 500, align: 'center' })
  doc.fontSize(14).font('Helvetica-Bold').text(winnerName ?? 'Not decided', 50, bannerY + 21, { width: 500, align: 'center' })
  doc.y = bannerY + 50
  doc.moveDown()
