...
import { insertNotificationsForProfiles, profileIdsForTeamRoster } from '../utils/athleteNotifications'
...
async function loadTeamSportSeason(teamId: string): Promise<
  | { ok: true; sport: string; season_id: string }
  | { ok: false; error: string }
> {
  const { data, error } = await supabase.from('teams').select('sport, season_id').eq('id', teamId).maybeSingle()
...
router.patch('/:id', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  ...
  const schema = z
    .object({
      ...
      best_of: z.union([z.literal(1), z.literal(3), z.literal(5), z.literal(7)]).optional().nullable(),
    })
    .refine(
      (o) =>
        o.name !== undefined ||
  ...
  try {
    ...
    if (body.description !== undefined) {
      updates.description =
        body.description === null || (typeof body.description === 'string' && body.description.trim() === '')
          ? null
          : body.description.trim()
    }
    if (body.category !== undefined) updates.category = body.category?.trim() || null
    if (body.table_tennis_format !== undefined) updates.table_tennis_format = body.table_tennis_format ?? null
    if (body.best_of !== undefined) updates.best_of = body.best_of ?? null

    const { data, error } = await supabase.from('events').update(updates).eq('id', id).select().single()
    if (error?.code === 'PGRST116' || !data) return res.status(404).json({ error: 'Event not found' })
...
router.post('/', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const effectiveBestOf = rest.sport === 'volleyball' || rest.sport === 'table-tennis' ? (best_of ?? null) : null

    const { data, error } = await supabase
      .from('events')
      .insert({ ...rest, category, description, table_tennis_format: ttFormat, best_of: effectiveBestOf, created_by: req.user!.id })
      .select()
      .single()
    ...
    await writeAuditLog({
      ...
      entityType: 'event',
      entityId: data.id,
      details: { name: body.name, sport: body.sport, season_id: body.season_id, format: body.format },
    })
...
router.delete('/:id', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  const { data: event } = await supabase.from('events').select('sport').eq('id', req.params.id).maybeSingle()
  if (!event) return res.status(404).json({ error: 'Event not found' })
  if (await respondIfSportForbidden(req, res, event.sport as string)) return
...
router.patch('/:id/status', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  const { newStatus } = req.body
  const { data: event } = await supabase.from('events').select('status, sport').eq('id', req.params.id).single()
  if (!event) return res.status(404).json({ error: 'Event not found' })
  if (await respondIfSportForbidden(req, res, event.sport as string)) return
  ...
  const { data, error } = await supabase.from('events').update({ status: newStatus }).eq('id', req.params.id).select().single()
  if (error) return res.status(500).json({ error: error.message })
...
router.post('/:id/participants', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  const { data: event } = await supabase.from('events').select('sport, season_id').eq('id', req.params.id).maybeSingle()
  if (!event) return res.status(404).json({ error: 'Event not found' })
  if (await respondIfSportForbidden(req, res, event.sport as string)) return
  ...
  try {
    ...
    await writeAuditLog({
      ...
      entityId: data.id,
      details: { event_id: req.params.id, participant_id: body.participant_id, participant_type: body.participant_type },
    })
...
router.post('/:id/participants/bulk', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  const { data: event } = await supabase.from('events').select('sport, season_id').eq('id', req.params.id).maybeSingle()
  if (!event) return res.status(404).json({ error: 'Event not found' })
  if (await respondIfSportForbidden(req, res, event.sport as string)) return
  ...
  try {
    ...
    res.status(400).json({ error: err instanceof Error ? err.message : 'Bulk add participants failed' })
  }
})

router.delete('/:id/participants/:participantId', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  const { data: event } = await supabase.from('events').select('sport').eq('id', req.params.id).maybeSingle()
  if (!event) return res.status(404).json({ error: 'Event not found' })
  if (await respondIfSportForbidden(req, res, event.sport as string)) return
...
router.patch('/:id/matches/:matchId/schedule', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  const { data: event } = await supabase.from('events').select('sport').eq('id', req.params.id).maybeSingle()
  if (!event) return res.status(404).json({ error: 'Event not found' })
  if (await respondIfSportForbidden(req, res, event.sport as string)) return
...
async function notifyMatchSchedule(match: {
  ...
  const recipientIds: string[] = (members ?? []).map((m: any) => m.athlete?.profile_id).filter(Boolean)
