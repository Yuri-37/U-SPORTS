...
router.get('/stats', requireAuth, requireRole('Admin', 'Organizer'), async (_req, res) => {
  const [athletes, events, seasons] = await Promise.all([
    supabase.from('athletes').select('id', { count: 'exact', head: true }).eq('season_status', 'active'),
...
router.patch('/organizers/:id', requireAuth, requireRole('Admin'), async (req: AuthRequest, res) => {
  const schema = z.object({
    role: z.enum(['Organizer', 'Coach']),
    department: z.enum(['SBMA', 'SECA', 'SASE', 'SHS']),
    assigned_sports: z.array(z.string()).min(1, 'Assign at least one sport'),
  ...
  try {
    ...
    await supabase.from('audit_logs').insert({
      ...
      details: { role: parsed.role, department: parsed.department, assigned_sports: parsed.assigned_sports },
    })
...
router.get('/seasons/:id/unfinished-matches', requireAuth, requireRole('Admin'), async (req: AuthRequest, res) => {
...
router.delete('/seasons/:id', requireAuth, requireRole('Admin'), async (req: AuthRequest, res) => {
  ...
  if (events && events.length > 0) {
    ...
    const { count } = await supabase
      ...
      .in('status', ['scheduled', 'live'])
    if ((count ?? 0) > 0) {
      return res.status(400).json({ error: 'Season has unfinished matches. Complete or cancel them before deleting.' })
    }
  }
