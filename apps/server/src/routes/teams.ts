...
router.get('/my-teams', requireAuth, requireRole('Organizer', 'Admin', 'Coach'), async (req: AuthRequest, res) => {
  const { data: staff } = await supabase.from('organizers').select('id').eq('profile_id', req.user!.id).maybeSingle()
...
router.get('/:id', async (req, res) => {
  ...
  const { data, error } = await supabase.from('teams').select(teamSelectEmbedded).eq('id', idParsed.data).maybeSingle()
  if (error) return res.status(500).json({ error: error.message })
...
router.post('/', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select()
      .single()
    if (error) return res.status(error.message.includes('violates foreign key') ? 400 : 500).json({ error: error.message })
...
router.patch('/:id', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  ...
  if (!parsed.success) {
    const msg = parsed.error.issues[0]?.message ?? 'Invalid request'
    return res.status(400).json({ error: msg.includes('uuid') ? 'Invalid season or captain id.' : msg })
  }

  const { data: existingTeam } = await supabase.from('teams').select('sport').eq('id', teamId.data).maybeSingle()
  ...
  const { data, error } = await supabase.from('teams').update(updates).eq('id', teamId.data).select().single()
  if (error) return res.status(error.message.includes('violates foreign key') ? 400 : 500).json({ error: error.message })
...
router.delete('/:id', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  ...
  const { data: team } = await supabase.from('teams').select('sport').eq('id', idParsed.data).maybeSingle()
  ...
  if ((count ?? 0) > 0) return res.status(409).json({ error: 'Remove this team from all events before deleting.' })
...
router.post('/:id/members', requireAuth, requireRole('Organizer', 'Admin', 'Coach'), async (req: AuthRequest, res) => {
  ...
  try {
    occupied = await loadOccupiedRosterKeys(team.season_id as string, team.sport as string, teamId)
  } catch (err: unknown) {
    return res.status(500).json({ error: err instanceof Error ? err.message : 'Roster check failed' })
  }
  ...
  if ((rosterCount ?? 0) >= maxRoster) {
    return res.status(400).json({ error: `Roster is full. Maximum ${maxRoster} players allowed for ${team.sport}.` })
  }
  ...
  if (athlete.sport !== team.sport) return res.status(400).json({ error: 'Athlete sport does not match this team' })
  ...
  if (occupied.athleteIds.has(athlete.id) || occupied.profileIds.has(athlete.profile_id)) {
    return res.status(400).json({ error: 'This player is already on another team for this sport and season.' })
  }
  ...
  const { data, error } = await supabase
    ...
    .single()
  if (error) {
    if (error.code === '23505') return res.status(400).json({ error: 'This athlete is already on this team.' })
...
router.delete('/:id/members/:membershipId', requireAuth, requireRole('Organizer', 'Admin', 'Coach'), async (req: AuthRequest, res) => {
  const teamId = z.string().uuid().parse(req.params.id)
  const membershipId = z.string().uuid().parse(req.params.membershipId)

  const { data: team } = await supabase.from('teams').select('sport').eq('id', teamId).maybeSingle()
  if (!team) return res.status(404).json({ error: 'Team not found' })
  if (await respondIfSportForbidden(req, res, team.sport as string)) return
...
router.post('/:id/members/bulk-remove', requireAuth, requireRole('Organizer', 'Admin', 'Coach'), async (req: AuthRequest, res) => {
  const teamId = z.string().uuid().parse(req.params.id)
  const parsed = z.object({ membership_ids: z.array(z.string().uuid()).min(1).max(100) }).safeParse(req.body)
  if (!parsed.success) return res.status(400).json({ error: 'Provide membership_ids (1-100 UUIDs).' })
  ...
  const { data: rows, error } = await supabase.from('team_members').select('id').eq('team_id', teamId).in('id', ids)
  if (error) return res.status(500).json({ error: error.message })
  const existingIds = new Set((rows ?? []).map((r: { id: string }) => r.id))
  if (existingIds.size !== ids.length) return res.status(400).json({ error: 'One or more roster rows are not on this team.' })

  const { error: delErr } = await supabase.from('team_members').delete().eq('team_id', teamId).in('id', ids)
...
router.post('/:id/coach', requireAuth, requireRole('Organizer', 'Admin', 'Coach'), async (req: AuthRequest, res) => {
  const { data: team, error: teamErr } = await supabase.from('teams').select('sport').eq('id', req.params.id).maybeSingle()
  if (teamErr) return res.status(500).json({ error: teamErr.message })
  if (!team) return res.status(404).json({ error: 'Team not found' })
  if (await respondIfSportForbidden(req, res, team.sport as string)) return
...
router.delete('/:id/coach', requireAuth, requireRole('Organizer', 'Admin', 'Coach'), async (req: AuthRequest, res) => {
  const { data: team } = await supabase.from('teams').select('sport').eq('id', req.params.id).maybeSingle()
  if (!team) return res.status(404).json({ error: 'Team not found' })
  if (await respondIfSportForbidden(req, res, team.sport as string)) return
  ...
  await supabase.from('team_coaches').delete().eq('organizer_id', staff.id).eq('team_id', req.params.id)
  await writeAuditLog({
    actorId: req.user!.id,
...
router.patch('/:id/lineup', requireAuth, requireRole('Organizer', 'Admin', 'Coach'), async (req: AuthRequest, res) => {
  const teamId = z.string().uuid().parse(req.params.id)
  const { data: team, error: teamErr } = await supabase.from('teams').select('id, sport, name').eq('id', teamId).maybeSingle()
  if (teamErr) return res.status(500).json({ error: teamErr.message })
  if (!team) return res.status(404).json({ error: 'Team not found' })
  if (await respondIfSportForbidden(req, res, team.sport as string)) return
  ...
  const filledSlots = parsed.data.slots.filter((s) => s.lineup_slot !== null).map((s) => s.lineup_slot as number)
  if (filledSlots.length > 0) {
    const maxSlot = Math.max(...filledSlots)
    const hardCap = getActiveSlots(team.sport, 'doubles')
    if (maxSlot > hardCap) {
      return res.status(400).json({ error: `Lineup slot ${maxSlot} exceeds max active slots (${hardCap}) for ${team.sport}` })
    }
  ...
  for (const row of beforeRows ?? []) beforeMap.set((row as { id: string; lineup_slot: number | null }).id, row.lineup_slot ?? null)

  const updates = await Promise.all(
    parsed.data.slots.map((s) =>
      supabase.from('team_members').update({ lineup_slot: s.lineup_slot }).eq('id', s.member_id).eq('team_id', teamId),
    ),
  )
  ...
  for (const row of afterRows ?? []) {
    const r = row as { id: string; lineup_slot: number | null; athlete?: { profile_id?: string | null } | null }
