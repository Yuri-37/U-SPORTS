    ...
    <Modal open={open} onClose={onClose} title="Match details" size="full">
      {loading ? (
        ...
        <div className="space-y-4 text-sm">
          ...
          {/* Full width, matching the Player Stats/Roster card below it — a
              narrower constrained card here left uneven whitespace next to
              the full-width table underneath. */}
          <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-elevated)] p-4">
            <p className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-2">
              Matchup
            </p>
            <p className="text-lg font-semibold">
              {nameA} <span className="text-[var(--text-muted)] font-normal text-base">vs</span>{' '}
              {nameB}
            </p>
            ...
            {scorePhaseReady ? (
              <div className="mt-4 pt-4 border-t border-[var(--border-subtle)] space-y-4">
                ...
                {showPeriodTable ? (
                      ...
                      <table className="w-full text-sm min-w-[280px]">
                        <thead>
                          <tr className="border-b border-[var(--border-subtle)] bg-[var(--surface-card)]">
                            <th className="text-left py-2.5 px-3 font-semibold text-[var(--text-muted)]">
                              {periodColumnLabel}
                            </th>
                            <th
                              className="text-right py-2.5 px-3 font-semibold tabular-nums max-w-[42%] truncate"
                              title={nameA}
                            >
                              {nameA}
                            </th>
                        ...
                        <tbody>
                          ...
                          <tr className="bg-[var(--surface-card)] font-semibold border-t border-[var(--border-subtle)]">
                            ...
                            <td className="py-2.5 px-3 text-right tabular-nums">
                              {breakdown!.totals.left}
                            </td>
                            <td className="py-2.5 px-3 text-right tabular-nums">
                              {breakdown!.totals.right}
                            </td>
                          </tr>
                        </tbody>
                      </table>
                  ...
                  <p className="text-xs text-[var(--text-muted)]">
                    Per-set or per-game points were not recorded; showing match outcome summary
                    above.
                  </p>
                ) : presentationFallback ? (
                  <>
                    <p className="text-xs text-[var(--text-muted)] mb-1">
                      {presentationFallback.subtitle}
                    </p>
                  ...
                  <p className="text-[var(--text-muted)] text-xs">
                    No score breakdown available for this sport yet.
                  </p>
                )}
              </div>
            ) : (
              <p className="mt-3 text-[var(--text-muted)] text-xs">
                {match.status === 'scheduled'
                  ? 'Scores appear once the match is underway.'
                  : 'No score rows yet.'}
              </p>
            )}
          </div>
          ...
          {match.status === 'live' ? (
            <Button
              ...
              onClick={() => window.open(`/jumbotron/${match.id}`, '_blank')}
            >
              Open live board
            </Button>
          ) : null}
