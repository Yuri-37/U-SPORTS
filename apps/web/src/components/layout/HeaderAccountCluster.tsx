...
export default function HeaderAccountCluster({ navVariant = 'app' }: Props) {
  ...
  const getNotifRoute = () => {
    if (scopedProfile?.role === 'Organizer' || scopedProfile?.role === 'Coach' || scopedProfile?.role === 'Admin') return '/organizer/announcements'
    return '/guest'
  }
  ...
  return (
    <>
      <div className="flex items-center gap-3">
        <div className="relative flex items-center gap-2 shrink-0" ref={usesNotificationPopover ? bellWrapRef : undefined}>
        <button
          type="button"
          onClick={() => {
          ...
          <div className="absolute right-0 top-full mt-1 w-80 max-h-[min(70vh,420px)] flex flex-col rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-card)] shadow-2xl z-50 overflow-hidden">
            <div className="flex items-center justify-between gap-2 px-3 py-2 border-b border-[var(--border-subtle)] shrink-0">
              ...
              <div className="flex items-center gap-1 shrink-0">
                {unreadCount > 0 && (
                  <Button size="sm" variant="ghost" className="text-xs h-7 px-2" onClick={() => void markAllRead(uid)}>
                    Read all
                  </Button>
                )}
            ...
            <div className="overflow-y-auto flex-1 p-2 space-y-1">
              {notifications.length === 0 ? (
                <p className="text-sm text-[var(--text-muted)] text-center py-8 px-2">No notifications yet.</p>
              ) : (
                notifications.map((n) => (
                  <button
                    key={n.id}
                    ...
                    className={`w-full text-left rounded-lg border px-3 py-2 transition-colors ${
                      !n.read ? 'border-[#0066FF]/30 bg-[#0066FF]/5' : 'border-[var(--border-subtle)] hover:bg-[var(--surface-elevated)]'
                    }`}
                  >
                    <div className="flex items-start gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-[var(--text-muted)] mt-0.5 line-clamp-2">{previewBody(n.body)}</p>
                        <p className="text-[10px] text-[var(--text-muted)] mt-1">{formatDateTime(n.created_at)}</p>
                      </div>
                    </div>
                  </button>
                ))
      ...
      <div className="relative shrink-0">
        ...
        >
          ...
          <span className="text-sm text-[var(--text-secondary)] hidden sm:block">{scopedProfile.full_name.split(' ')[0]}</span>
        ...
        {menuOpen && (
          <div className="absolute right-0 top-full mt-1 w-48 bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-xl shadow-2xl py-1 z-50">
            <div className="px-3 py-2 border-b border-[var(--border-subtle)]">
              ...
              <p className="text-xs text-[var(--text-muted)] capitalize">{scopedProfile.role.replace('_', ' ')}</p>
            </div>
      ...
      >
        {detailNotif && uid && (
          <div className="space-y-4">
            <p className="text-xs text-[var(--text-muted)]">{formatDateTime(detailNotif.created_at)}</p>
            <p className="text-sm text-[var(--text-secondary)] whitespace-pre-wrap">{detailNotif.body}</p>
            {notifDeleteConfirm ? (
              <>
                <Alert type="warning">
                  Delete this notification permanently? You cannot restore it from the inbox afterward.
                </Alert>
                <div className="flex gap-2 justify-end pt-2 flex-wrap">
                  <Button variant="secondary" size="sm" onClick={() => setNotifDeleteConfirm(false)}>
                    Cancel
                  </Button>
                  <Button
                    variant="danger"
      ...
      <Modal open={clearAllOpen} onClose={() => setClearAllOpen(false)} title="Clear all notifications" size="md">
        {uid && (
          <div className="space-y-4">
