...
import { formatEnumLabel, getSportIcon, getSportLabel, organizerEventStatusLabel, formatDateTime } from '../../lib/utils'
...
export default function OrganizerDashboard() {
        ...
        .then(async ({ data }) => {
          ...
          const [liveRes, upcomingRes] = await Promise.all([
            supabase.from('matches').select('*').eq('status', 'live')
              .or(teamIds.map((id: string) => `participant_a_id.eq.${id},participant_b_id.eq.${id}`).join(',')),
            supabase.from('matches').select('*, event:events(name, sport)').eq('status', 'scheduled')
              .or(teamIds.map((id: string) => `participant_a_id.eq.${id},participant_b_id.eq.${id}`).join(','))
          ...
          const allIds = [...new Set([
            ...(liveRes.data ?? []).flatMap((m: Match) => [m.participant_a_id, m.participant_b_id]),
            ...(upcomingRes.data ?? []).flatMap((m: Match) => [m.participant_a_id, m.participant_b_id]),
  ...
  useEffect(() => {
    if (isCoach) return
    const ids = [...new Set(liveMatches.flatMap(m => [m.participant_a_id, m.participant_b_id].filter((x): x is string => !!x)))]
    if (ids.length === 0) { setLiveMatchLabels({}); return }
    let cancelled = false
    void fetchParticipantLabels(ids).then(map => { if (!cancelled) setLiveMatchLabels(map) })
    ...
    return (
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-[var(--text-muted)] text-sm mt-1">
            ...
            {(organizer?.assigned_sports ?? []).length > 0 && (
              <> · {(organizer!.assigned_sports as string[]).map(s => getSportLabel(s as any)).join(', ')}</>
            )}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <StatCard label="Teams" value={loading ? '—' : coachTeams.length} subValue="Assigned to you" />
          <StatCard label="Athletes" value={loading ? '—' : coachAthleteCount} subValue="Across your teams" />
          <StatCard label="Live now" value={loading ? '—' : coachLiveMatches.length} subValue="Matches in progress" />
              ...
              {coachLiveMatches.map(m => (
                <div key={m.id} className="flex items-center justify-between gap-3 text-sm">
                  <span className="text-[var(--text-secondary)]">
                    {coachMatchLabels[m.participant_a_id ?? ''] ?? '—'} vs {coachMatchLabels[m.participant_b_id ?? ''] ?? '—'}
                  </span>
        ...
        <div>
          ...
          {loading ? (
            ...
            <p className="text-sm text-[var(--text-muted)]">No teams assigned yet. An organizer will add you to a team.</p>
          ) : (
              ...
              {coachTeams.map((t: any) => (
                ...
                >
                  <Card className="hover:border-[var(--accent-default)]/40 transition-colors h-full">
                    ...
                    <p className="text-xs text-[var(--text-muted)]">{getSportLabel(t.sport)}{t.department ? ` · ${t.department}` : ''}</p>
                    {t.season && (
                ...
                <Card key={m.id} className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-[var(--text-muted)] mt-0.5">
                      {(m as any).event?.name ?? 'Event'} · {m.scheduled_at ? formatDateTime(m.scheduled_at) : 'Date TBD'}
                    </p>
                  </div>
        ...
        <div>
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            {[
              { icon: Dumbbell, label: 'Teams', sub: 'Manage rosters and lineups', to: '/organizer/teams' },
              { icon: UserCheck, label: 'Athletes', sub: 'View your sport\'s athletes', to: '/organizer/athletes' },
              { icon: BarChart3, label: 'Analytics', sub: 'Season stats and insights', to: '/organizer/analytics' },
    ...
    <div className="space-y-6">
      ...
      {liveMatches.length > 0 && (
        <div className="bg-[var(--danger)]/10 border border-[var(--danger)]/30 rounded-xl p-4">
          ...
          <div className="space-y-2">
            {liveMatches.map(m => {
              const la = m.participant_a_id ? liveMatchLabels[m.participant_a_id] ?? `Team ${m.participant_a_id.slice(0, 6)}` : 'TBD'
              const lb = m.participant_b_id ? liveMatchLabels[m.participant_b_id] ?? `Team ${m.participant_b_id.slice(0, 6)}` : 'TBD'
              return (
                <div key={m.id} className="flex items-center justify-between gap-3 flex-wrap">
                  <span className="text-sm min-w-0">{la} <span className="text-[var(--text-muted)] font-normal">vs</span> {lb}</span>
                  <div className="flex gap-2 shrink-0">
                    <Button size="sm" onClick={() => navigate(`/organizer/scoring/${m.id}`)}>Score Now</Button>
                    <Button size="sm" variant="secondary" onClick={() => window.open(`/jumbotron/${m.id}`, '_blank')}>Jumbotron</Button>
                  </div>
                </div>
              )
            })}
        ...
        <Card>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              const Icon = a.icon
              return (
                <button key={a.label} onClick={a.action} className="flex flex-col items-center gap-2 p-4 rounded-xl border border-[var(--border-subtle)] hover:border-[var(--accent-default)]/50 hover:bg-[var(--accent-default)]/5 transition-all">
