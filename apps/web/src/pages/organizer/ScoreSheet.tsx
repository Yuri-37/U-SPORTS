...
export default function ScoreSheet() {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const safeName = (s: string) => s.replace(/[^a-zA-Z0-9 _-]/g, '').trim().replace(/\s+/g, '-')
  ...
  if (!match.finalized_at) {
    return (
      <div className="max-w-xl mx-auto text-center py-20 space-y-6">
        <Alert type="warning">This match hasn't been finalized yet — finalize it from Post-Game Review first.</Alert>
        <Button onClick={() => navigate(`/organizer/match-review/${matchId}`)}>Go to Post-Game Review</Button>
      </div>
    )
  }
  ...
  const finalScoreLabel = sport === 'volleyball'
  ...
  const winnerName = winnerId === match.participant_a_id ? nameA : winnerId === match.participant_b_id ? nameB : null
  ...
  const finalColumnA = sport === 'volleyball' ? (scoreA?.sets_won ?? 0) : sport === 'table-tennis' ? (scoreA?.games_won ?? 0) : bbTotal(scoreA)
  const finalColumnB = sport === 'volleyball' ? (scoreB?.sets_won ?? 0) : sport === 'table-tennis' ? (scoreB?.games_won ?? 0) : bbTotal(scoreB)
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-3">
        ...
        <Button size="sm" icon={<Download className="w-3 h-3" />} loading={downloadingPdf} onClick={handleDownloadPdf}>
          Download PDF
        </Button>
      </div>
      ...
      <Card>
        <div className="-mx-4 -mt-4 mb-4 px-4 py-2 rounded-t-xl bg-[var(--surface-elevated)] border-b border-[var(--border-subtle)]">
          <h3 className="font-semibold flex items-center gap-2"><FileText className="w-4 h-4" /> Match Details</h3>
        </div>
        <dl className="grid grid-cols-2 gap-3 text-sm">
          <div>
            <dt className="text-[var(--text-muted)] text-xs">Event</dt>
            <dd>{match.event?.name ?? '—'}{match.bracket?.round ? ` · Round ${match.bracket.round}` : ''}</dd>
          </div>
          <div>
      ...
      <Card>
        ...
        <p className="text-3xl font-black text-center py-2">{nameA} {finalScoreLabel} {nameB}</p>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr>
                  ...
                  {periodBreakdown.rows.map((row) => (
                    <th key={row.label} className="border border-[var(--border-subtle)] bg-[var(--surface-elevated)] p-2 text-center text-xs uppercase tracking-wide text-[var(--text-muted)]">
                      {row.label}
                    </th>
                  ))}
              ...
              <tbody>
                <tr className={winnerId === match.participant_a_id ? 'bg-[var(--success)]/10' : ''}>
                  <td className="border border-[var(--border-subtle)] p-2 font-semibold">{nameA}</td>
                  {periodBreakdown.rows.map((row) => (
                    <td key={row.label} className="border border-[var(--border-subtle)] p-2 text-center">{row.left}</td>
                  ))}
                  <td className="border border-[var(--border-subtle)] p-2 text-center font-bold">{finalColumnA}</td>
                </tr>
                <tr className={winnerId === match.participant_b_id ? 'bg-[var(--success)]/10' : ''}>
                  <td className="border border-[var(--border-subtle)] p-2 font-semibold">{nameB}</td>
                  {periodBreakdown.rows.map((row) => (
                    <td key={row.label} className="border border-[var(--border-subtle)] p-2 text-center">{row.right}</td>
                  ))}
                  <td className="border border-[var(--border-subtle)] p-2 text-center font-bold">{finalColumnB}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="mt-4 border-2 border-[var(--success)] rounded-lg bg-[var(--success)]/10 py-3 text-center">
          <p className="text-xs uppercase tracking-widest text-[var(--success)] font-semibold">Victorious Team</p>
      ...
      <Card>
        ...
        {playerStats.length === 0 ? (
          <p className="text-center py-8 text-[var(--text-muted)]">No roster athletes are linked to these match participants.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {playerStats.map(ps => {
                  const teamLabel = ps.team_name?.trim() ?? (ps.participant_side === 'a' ? nameA : ps.participant_side === 'b' ? nameB : 'Team')
                  return (
                    <tr key={ps.athlete_id} className="border-t border-[var(--border-subtle)]">
                      <td className="py-2 pr-3 w-36 min-w-36 max-w-36 align-top text-[var(--text-secondary)]">
                        <span className="line-clamp-2 break-words text-xs sm:text-sm" title={teamLabel}>{teamLabel}</span>
                      </td>
                      <td className="py-2 pr-3 font-medium min-w-[8rem]">
                        {ps.athlete?.profile?.full_name ?? `Athlete ${ps.athlete_id.slice(0, 6)}`}
                      </td>
                      {statDefs.map(s => (
                        <td key={s.key} className="py-2 px-2 text-center">{ps.stats?.[s.key] ?? 0}</td>
                      ))}
                    </tr>
                  )
                })}
