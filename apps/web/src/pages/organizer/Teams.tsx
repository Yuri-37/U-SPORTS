...
import { Button, Card, Modal, Input, Select, EmptyState, Alert, Skeleton, Badge } from '../../components/ui'
...
export default function OrganizerTeams() {
  ...
  const { canConfigureSport, sportOptionsForForms, hasFullSportAccess, assignedSports } = useOrganizerSportScope()
  ...
  const [removeMemberConfirm, setRemoveMemberConfirm] = useState<{ membershipId: string; label: string } | null>(null)

  const [coachLeaveConfirm, setCoachLeaveConfirm] = useState<{ teamId: string; teamName: string } | null>(null)
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      ...
      document.getElementById(`team-card-${highlightTeamId}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    ...
    api
      ...
      .catch(() => {
        ...
        if (!cancelled) {
          ...
          setEligibleAthletesFetchError('Could not load athletes. Check the API server and try again.')
  ...
  const openEdit = async (team: Team & { members: any[] }) => {
    ...
    if (!merged.some((s) => s.id === team.season_id)) {
      ...
      const { data } = await supabase.from('seasons').select('*').eq('id', team.season_id).maybeSingle()
    ...
    try {
      ...
      setEditError('Could not load full team from the server. You can still edit; try Save, or check VITE_API_URL and that the API is running.')
  ...
  const handleCreate = async () => {
    ...
    if (!form.season_id) {
      ...
      setError('Select a season. If none appear, ask a Super Admin to create one (Super Admin → Seasons) — new seasons start as Draft until Activated.')
        ...
        setError(
          ...
          apiErr ??
            ...
            (e.code === 'ECONNABORTED'
              ...
              : e.message || 'Network error — is the API running? Set VITE_API_URL in apps/web/.env (e.g. http://localhost:3001/api)')
  ...
  const handleSetSlot = async (memberId: string, slot: number | null) => {
    ...
    try {
      ...
      if (axios.isAxiosError(e)) {
        const d = e.response?.data
        const apiErr = d && typeof d === 'object' && 'error' in d ? String((d as { error: string }).error) : null
  ...
  const toggleRosterMemberSelect = (id: string) => {
    ...
    setSelectedRosterMemberIds((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]))
  ...
  const toggleAthleteAddSelect = (id: string) => {
    ...
    setSelectedAthleteIdsToAdd((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]))
  ...
  const executeBulkRemoveMembers = async () => {
    ...
    try {
      ...
      await api.post(`/teams/${editTeam.id}/members/bulk-remove`, { membership_ids: bulkRemoveMembershipIds })
  ...
  const bulkBenchSelected = async () => {
    ...
    const toBench = members.filter((m: any) => selectedRosterMemberIds.includes(m.id) && m.lineup_slot != null)
  ...
  const bulkActivateSelected = async () => {
    ...
    const freeSlots = Array.from({ length: maxActive }, (_, i) => i + 1).filter((s) => !usedSlots.has(s))

    const toPromote = members.filter((m: any) => selectedRosterMemberIds.includes(m.id) && m.lineup_slot == null)

    if (toPromote.length === 0) return

    if (toPromote.length > freeSlots.length) {

      setEditError(`Only ${freeSlots.length} active slot(s) open — bench someone first or select fewer players.`)

      return

    }

    const slots = toPromote.map((m: any, idx: number) => ({ member_id: m.id, lineup_slot: freeSlots[idx] }))
  ...
  const addSelectedAthletesBulk = async () => {
    ...
    if (currentCount + selectedAthleteIdsToAdd.length > maxRoster) {
      ...
      setEditError(`Roster limit exceeded. This team can have at most ${maxRoster} members (currently ${currentCount}).`)
  ...
  const filteredTeams = useMemo(() => {
    ...
    return teams.filter((t) => {
      ...
      const haystack = `${t.name} ${sportLabel} ${sportSlug} ${seasonName} ${t.department ?? ''}`.toLowerCase()
  ...
  const teamListFiltersActive =
    ...
    teamListSearch.trim() !== '' || teamSeasonFilter !== '' || teamDepartmentFilter !== '' || teamSportFilter !== ''
  ...
  return (
    ...
    <div className="space-y-6">
        ...
        <div>
          ...
          {!hasFullSportAccess && assignedSports.length > 0 ? (
            ...
            <p className="text-xs text-[var(--text-muted)] mt-1 max-w-xl">
              ...
              You can edit teams for {assignedSports.map((s) => getSportLabel(s as any)).join(', ')} only. Other sports are view-only.
      ...
      {loading ? (
        ...
        <>
          ...
          {filteredTeams.length === 0 ? (
            ...
            const isCoach = team.coaches?.some((c: any) => c.organizer?.profile_id === myProfileId)
      ...
      >
        ...
        {seasons.length === 0 && (
          ...
          <Alert type="warning" className="mb-4">
            ...
            No <strong>Draft</strong> or <strong>Active</strong> seasons yet. Sign in as Super Admin → Seasons → Create, then Activate (or teams can attach to Draft for testing).
        ...
        <div className="space-y-4">
          ...
          <Select label="Season" value={form.season_id} onChange={e => setForm(f => ({...f, season_id: e.target.value}))}
            ...
            options={[
              ...
              { value: '', label: seasons.length ? 'Select season...' : 'No seasons — create one in Super Admin' },
          ...
          <Button type="button" className="w-full" loading={creating} disabled={!seasons.length || !canCreateTeams} onClick={() => void handleCreate()}>
      ...
      >
        ...
        {editTeam ? (
          <>
        {editError && <Alert type="danger" className="mb-4">{editError}</Alert>}

        <div className="space-y-4">

          <Input label="Team Name" value={editForm.name} onChange={e => setEditForm(f => ({ ...f, name: e.target.value }))} />
          ...
          <div className="rounded-xl border border-[var(--border-subtle)] p-4 space-y-3">
            ...
            {(() => {
              ...
              const maxActive = activeSlotsFor(sport)
              const activeMembers = members.filter((m: any) => m.lineup_slot != null).sort((a: any, b: any) => a.lineup_slot - b.lineup_slot)
              const benchMembers = members.filter((m: any) => m.lineup_slot == null)
              const usedSlots = new Set(activeMembers.map((m: any) => m.lineup_slot as number))
              const nextFreeSlot = Array.from({ length: maxActive }, (_, i) => i + 1).find((s) => !usedSlots.has(s)) ?? null
                ...
                <>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-[var(--text-muted)]">
                      {activeMembers.length}/{maxActive} active · {members.length}/{maxRoster} total
                      {sport === 'table-tennis' ? <span className="ml-1 opacity-60">(format set per event)</span> : null}
                    </span>
                  </div>
                  ...
                  {selectedRosterMemberIds.length > 0 ? (
                    <div className="flex flex-wrap items-center gap-2 rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-raised)] px-3 py-2">
                      <span className="text-xs text-[var(--text-muted)]">{selectedRosterMemberIds.length} selected</span>
                      <Button type="button" size="sm" variant="secondary" loading={rosterBulkBusy} onClick={() => void bulkBenchSelected()}>
                        Bench
                      </Button>
                      <Button type="button" size="sm" variant="secondary" loading={rosterBulkBusy} onClick={() => void bulkActivateSelected()}>
                        Activate
                      </Button>
                      <Button
                        type="button"
                      ...
                      <Button type="button" size="sm" variant="ghost" className="ml-auto" onClick={() => setSelectedRosterMemberIds([])}>
                        Clear
                      </Button>
                    </div>
                  ) : null}
                  ...
                  {isFull ? (
                    <Alert type="warning" className="text-xs">Roster is full ({maxRoster}/{maxRoster}). Remove a player before adding new ones.</Alert>
                  ) : null}

                  {/* Active slots */}
                  <div className="space-y-1">
                    <p className="text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wide">Active</p>
                    {activeMembers.length === 0 ? (
                      <p className="text-xs text-[var(--text-muted)]">No active players yet — promote someone from the bench.</p>
                    ) : null}
                    <ul className="space-y-1.5">
                      {activeMembers.map((m: any) => (
                        <li key={m.id} className="flex items-center gap-2 text-sm rounded-lg bg-[var(--surface-raised)] px-3 py-1.5">
                          <input
                            type="checkbox"
                          ...
                          />
                          <span className="w-5 h-5 rounded-full bg-[#0066FF]/20 text-[#4C9FFF] text-xs font-bold flex items-center justify-center shrink-0">{m.lineup_slot}</span>
                          ...
                          <Button
                            ...
                            onClick={() => setRemoveMemberConfirm({ membershipId: m.id, label: memberLabel(m) })}
                          >Remove</Button>
                        </li>
                      ))}
                    </ul>
                  ...
                  {benchMembers.length > 0 ? (
                    <div className="space-y-1">
                      <p className="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wide">Bench</p>
                      <ul className="space-y-1.5">
                        {benchMembers.map((m: any) => (
                          <li key={m.id} className="flex items-center gap-2 text-sm rounded-lg border border-[var(--border-subtle)] px-3 py-1.5 opacity-75">
                            <input
                              type="checkbox"
                              className="size-4 shrink-0 rounded border-[var(--border-subtle)] bg-[var(--surface-base)] accent-[#0066FF]"
                              checked={selectedRosterMemberIds.includes(m.id)}
                            ...
                            {!!(m.student_profile_id || (m.student_profile && !m.athlete)) ? (
                              <Badge size="sm" variant="info">Tryout</Badge>
                            ) : null}
                            <Button
                              type="button" size="sm" variant="secondary"
                              loading={savingLineupId === m.id}
                              disabled={nextFreeSlot === null}
                              title={nextFreeSlot === null ? `All ${maxActive} active slots are filled` : `Set as active (slot ${nextFreeSlot})`}
            ...
            <p className="text-sm font-semibold text-[var(--text-primary)] sr-only">Roster (add)</p>
            ...
            <p className="text-xs text-[var(--text-muted)] leading-relaxed">
              Add approved athletes only. People already rostered on another team in this season (same sport) are hidden.
            </p>
            ...
            {editForm.sport !== editTeam.sport ? (
              ...
              <p className="text-xs text-[#FFB800]">
                ...
                You changed the sport above — <strong>Save</strong> the team first so the roster list and server stay in sync.
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:items-end pt-2 border-t border-[var(--border-subtle)]">
                <div className="flex-1 min-w-0 space-y-2">
                  <p className="text-xs font-medium text-[var(--text-secondary)]">Approved roster athlete (official)</p>
                  ...
                  {eligibleLoading ? (
                      ...
                      {eligibleAthletes.length === 0 ? (
                        <>
                          <strong className="text-[var(--text-secondary)]">
                            No approved athletes for {getSportLabel(editTeam.sport as any)} in the database yet.
                          </strong>{' '}
                          Approve athletes under <strong className="text-[var(--text-secondary)]">Athletes</strong>{' '}
                          (same sport as this team), or run <strong className="text-[var(--text-secondary)]">pnpm db:seed:dev</strong> for dev seed accounts.
                          Medical clearance can be completed there before official matches.
                          <div className="mt-2">
                            <Button type="button" size="sm" variant="outline" onClick={() => navigate('/organizer/athletes')}>
                              Open Athletes
                            </Button>
                          </div>
                        </>
                        ...
                        <>
                          ...
                          There are {eligibleAthletes.length} approved {getSportLabel(editTeam.sport as any)} player(s), but each may only be on one{' '}
                          official roster per season for this sport. They&apos;re either already on{' '}
                          <strong className="text-[var(--text-secondary)]">this team</strong> or on{' '}
                          <strong className="text-[var(--text-secondary)]">another team</strong> for the same season. Remove someone from the other roster or pick another season.
                    ...
                    <div className="space-y-3">
                      <p className="text-xs text-[var(--text-muted)]">Select athletes, then add in one action.</p>
                      <div className="max-h-44 overflow-y-auto rounded-lg border border-[var(--border-subtle)] divide-y divide-[var(--border-subtle)]">
                        {officialAthletePickRows.map((a) => {
                          const label = a.profile?.full_name?.trim() || a.student_id || a.id.slice(0, 8)
                          return (
                            <label
                              key={a.id}
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        >
                          Add selected{selectedAthleteIdsToAdd.length ? ` (${selectedAthleteIdsToAdd.length})` : ''}
                        </Button>
                        <Button
                          type="button"
                          variant="secondary"
          ...
          <div className="flex gap-2 pt-2">
            ...
            <Button type="button" className="flex-1" loading={savingEdit} onClick={() => void handleUpdate()}>
        ...
        {bulkRemoveMembershipIds && bulkRemoveMembershipIds.length > 0 ? (
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm text-[var(--text-secondary)]">
              ...
              <span className="font-semibold text-[var(--text-primary)]">{bulkRemoveMembershipIds.length}</span> players from this roster?
            ...
            <div className="flex gap-2">
              ...
              <Button type="button" variant="secondary" className="flex-1" disabled={rosterBulkBusy} onClick={() => setBulkRemoveMembershipIds(null)}>

                Cancel

              </Button>

              <Button type="button" variant="danger" className="flex-1" loading={rosterBulkBusy} onClick={() => void executeBulkRemoveMembers()}>
        ...
        {removeMemberConfirm && (
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm text-[var(--text-secondary)]">
              ...
              Remove <span className="font-semibold text-[var(--text-primary)]">{removeMemberConfirm.label}</span> from this team roster?

              Line-up slots update immediately; you can add them again later if roster space allows.
            ...
            <div className="flex gap-2">
              ...
              <Button type="button" variant="secondary" className="flex-1" disabled={!!removingMembershipId} onClick={() => setRemoveMemberConfirm(null)}>

                Cancel

              </Button>

              <Button type="button" variant="danger" className="flex-1" loading={!!removingMembershipId} onClick={() => void removeMember(removeMemberConfirm.membershipId)}>
        ...
        {coachLeaveConfirm && (
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm text-[var(--text-secondary)]">
              ...
              Stop coaching <span className="font-semibold text-[var(--text-primary)]">{coachLeaveConfirm.teamName}</span>? Another organizer can assign themselves as coach afterward.
            ...
            <div className="flex gap-2">
              ...
              <Button type="button" variant="secondary" className="flex-1" disabled={coachBusy} onClick={() => setCoachLeaveConfirm(null)}>

                Cancel

              </Button>

              <Button type="button" variant="danger" className="flex-1" loading={coachBusy} onClick={() => void confirmLeaveCoachRole()}>
      ...
      >
        ...
        {deleteTarget && (
          ...
          <p className="text-sm text-[var(--text-secondary)] mb-4">
            ...
            Delete <span className="font-semibold text-[var(--text-primary)]">{deleteTarget.name}</span>? This cannot be undone. Teams registered on an event must be removed from the event first.

          </p>

        )}

        {deleteError && <Alert type="danger" className="mb-4">{deleteError}</Alert>}
        ...
        <div className="flex gap-2">
          ...
          <Button type="button" variant="danger" className="flex-1" loading={deleting} onClick={() => void handleDelete()}>
