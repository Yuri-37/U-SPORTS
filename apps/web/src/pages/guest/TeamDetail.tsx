...
export default function GuestTeamDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase
        .from('team_members')
        .select('athlete:athletes(id, position, jersey_number, profile:profiles!athletes_profile_id_fkey(full_name))')
        .eq('team_id', id),
      supabase
        .from('team_coaches')
        .select('organizer:organizers(profile:profiles!organizers_profile_id_fkey(full_name))')
        .eq('team_id', id),
      supabase.from('team_season_stats').select('*').eq('team_id', id).order('updated_at', { ascending: false }).limit(1).maybeSingle(),
      supabase
        .from('matches')
      ...
      if (opponentIds.size > 0) {
        const [teamsRes, athletesRes] = await Promise.all([
          ...
          supabase.from('athletes').select('id, profile:profiles!athletes_profile_id_fkey(full_name)').in('id', [...opponentIds]),
        ])
      ...
      if (!cancelled) {
        setMatches(
          matchRows.map((row: any) => {
            const oppId = row.participant_a_id === id ? row.participant_b_id : row.participant_a_id
  ...
  if (loading)
    return (
      <div className="max-w-xl mx-auto px-4 py-8 space-y-4">
        <Button type="button" variant="ghost" size="sm" icon={<ArrowLeft className="w-4 h-4" />} className="-ml-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)]" onClick={goBack}>
          Back
        </Button>
      ...
      {stats && (
        <Card>
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <p className="text-2xl font-black font-[Barlow_Condensed] text-[var(--success)]">{wins}</p>
            ...
            {matches.map((m) => {
              ...
              const scoreLine =
                (m.status === 'completed' || m.status === 'live') && myScore && oppScore
                  ...
                  : null
              return (
                <div key={m.id} className="flex items-center justify-between py-2 border-b border-[var(--border-subtle)] last:border-0">
                  <div>
                    <p className="font-medium">vs {m.opponentName}</p>
                    <p className="text-xs text-[var(--text-muted)]">{m.event?.name}</p>
                    {m.scheduled_at && <p className="text-xs text-[var(--text-muted)]">{formatDateTime(m.scheduled_at)}</p>}
                  </div>
                  {scoreLine ? (
                    <span className="font-black text-lg">{scoreLine}</span>
                  ) : (
                    <span className="text-sm text-[var(--text-muted)]">{matchStatusLabel(m.status)}</span>
                  )}
                </div>
              )
            })}
