    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      >
        <div
          ...
          style={{
            backgroundImage: 'radial-gradient(circle at 50% 0%, #FFD700 1px, transparent 1px)',
            backgroundSize: '30px 30px',
          }}
        />
        <div className="relative">
          <p className="text-[var(--school-secondary)] font-bold text-sm uppercase tracking-widest mb-2">
      ...
      {championSpotlights.length > 0 && (
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)] mb-4">
            Knockout finals from completed competitions. Tap an event for brackets and match
            history.
          </p>
      ...
      {[institution?.address, institution?.region].some((v) => v && v.trim()) && (
        <div className="flex items-center justify-center gap-2 text-sm text-[var(--text-muted)] pt-2">
          ...
          <span>
            {[institution?.address, institution?.region].filter((v) => v && v.trim()).join(', ')}
          </span>
        </div>
      )}
    </div>
