...
export default function SuperAdminSeasons() {
  ...
  const [transitionConfirm, setTransitionConfirm] = useState<{ id: string; name: string; nextStatus: string; unfinishedMatches?: number } | null>(
    null,
  )
  ...
  const transitionDescription = (next: string) => {
    ...
    if (next === 'completed') return 'Mark this season as completed? This action closes the season — events and standings will be locked.'
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => { setDeleteError(''); setDeleteConfirm({ id: s.id, name: s.name }) }}
      ...
      <Modal
        ...
        onClose={() => { if (!transitionBusy) setTransitionConfirm(null) }}
        title="Confirm season change"
        size="md"
      >
        {transitionConfirm && (
          <div className="space-y-4">
            {transitionConfirm.nextStatus === 'completed' && (transitionConfirm.unfinishedMatches ?? 0) > 0 && (
              <Alert type="danger">
                <span className="font-semibold">⚠ {transitionConfirm.unfinishedMatches} unfinished {transitionConfirm.unfinishedMatches === 1 ? 'match' : 'matches'}</span>
                <span className="block mt-1 text-sm">There are still scheduled or live matches in this season. Completing it now will leave those matches unresolved. Are you sure you want to continue?</span>
              </Alert>
            )}
            <Alert type="warning">
      ...
      <Modal
        ...
        onClose={() => { if (!deleteBusy) { setDeleteConfirm(null); setDeleteError('') } }}
        title="Delete season"
        size="md"
      >
        {deleteConfirm && (
          <div className="space-y-4">
            ...
            <Alert type="danger">
              ...
              <span className="block mt-1 text-sm">This will permanently delete the season and all associated events and brackets. This cannot be undone.</span>
            </Alert>
            <div className="flex justify-end gap-2">
              <Button variant="secondary" onClick={() => { setDeleteConfirm(null); setDeleteError('') }} disabled={deleteBusy}>
                Cancel
              </Button>
              <Button
                loading={deleteBusy}
