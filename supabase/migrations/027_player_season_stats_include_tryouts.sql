-- Include tryout/scrimmage matches in player_season_stats.games_played and stat rollups.
-- Athletes see the same game count as their schedule modal (which lists all season matches).
-- Team win–loss (team_season_stats) remains tryout-free — enforced in apps/server scoring routes.
